...
def assimilate_health_data_node(state: AgentState) -> dict:
    """
    Checks for health alerts and saves them as facts in ChromaDB.
    This cures the agent's amnesia about health data.
    """
    ...
    for alert in health_alerts:
        # Format the alert into a declarative fact
    ...
    if facts_to_add and facts_collection:
        try:
            # Generate unique IDs for each new fact
            ...
            # Optionally, add to the session log for UI display
            current_episodic_log = state.get("episodic_memory_session_log", [])
...
def health_check_node(state: AgentState) -> dict:
    """A new node that runs on every turn to check health data."""
     #no longer used.
    # This node doesn't need to print to the console unless an issue is found,
    # as the underlying tool already does.
    alerts = check_health_data()
...
def generate_response_node(state: AgentState) -> dict:
    """
    Generates a response using a structured "mission" prompt to ensure
    the LLM follows turn-specific instructions.
    """
    ...
    # --- Start with the Base Prompt: Identity and Static Context ---
    ...
    # --- Now, define the SINGLE, OVERRIDING mission for THIS turn ---
    turn_specific_task = ""
    if retrieved_context_str:
        # QA Mode is a special case and will use its own separate, focused prompt.
        ...
        # Append the final mission to the system prompt
...
if prompt := st.chat_input(f"Chat with {AGENT_NAME}..."):
    ...
    with st.spinner(f"{AGENT_NAME} is thinking..."):
        # 1. Run the health check HERE, outside the cached graph.
        print("--- Checking Health Data (Live from Streamlit loop) ---")
        live_health_alerts = check_health_data() # This calls your tool directly

        # 2. Manually inject the LIVE data into the state for this turn.
