...
def schedule_event(user_input: str, conversation_history: List[BaseMessage]) -> str:
    ...
    prompt_template = PromptTemplate.from_template(
        ...
        - 'date_time_string': The specific part of the request describing the date and time.
        - 'location': The location, if mentioned.

        You MUST use the "Recent Conversation History" to understand relative references like "that day", "after the match", or "in the morning of that event".
    ...
    try:
        ...
        if not event_data.get("title") or not event_data.get("date_time_string"):
    ...
    try:
        raw_date_str = event_data['date_time_string']
        # The 'PREFER_DATES_FROM': 'future' setting helps resolve ambiguity.
        # It will correctly interpret "8 pm" as the next 8 pm, not one in the past.
        settings = {'PREFER_DATES_FROM': 'future'}
        start_dt = dateparser.parse(raw_date_str, settings=settings)
        
        if not start_dt:
            raise ValueError(f"Could not parse date string: {raw_date_str}")
        
        print(f"  Parsed datetime object: {start_dt}")
        
        if start_dt.hour == 0 and start_dt.minute == 0:
            start_dt = start_dt.replace(hour=10, minute=0)
            print(f"  No time found, defaulting to: {start_dt}")
